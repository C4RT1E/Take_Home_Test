      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 sm:py-2 rounded-xl bg-slate-900/60 border border-slate-800 text-slate-200 placeholder-slate-500 text-base sm:text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all min-h-[42px]"
          />
        </div>

        {/* Filter Tabs */}
        <div className="flex items-center p-1 rounded-xl bg-slate-900/80 border border-slate-800 gap-1 w-full sm:w-auto overflow-x-auto justify-between sm:justify-start">
          {filterTabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative px-3 py-2 sm:py-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer flex-1 sm:flex-initial min-h-[36px] ${
              ...
              >
                ...
                <span
                  className={`relative z-10 px-1.5 py-0.5 rounded-full text-[10px] ${
                    isActive ? 'bg-indigo-500/20 text-indigo-300 font-bold' : 'bg-slate-800 text-slate-500'
                  }`}
                >
                  {tab.count}
                </span>
